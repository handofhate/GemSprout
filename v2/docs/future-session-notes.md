...
- Device testers can leave the seeded family from `Settings -> Account & Security -> Join Different Family`; it signs out, clears local lock state, and returns to the front door even on `source=firestore`. Use that path before the real-family onboarding pass.
